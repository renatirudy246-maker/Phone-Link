"""Helper script to detect and verify Ground Truth corners with high-resolution edge analysis.
Generates tools/scanpoc/out/gt-check/*.gt-overlay.png for visual verification.
"""
import json
import math
import os
...
out_dir = "tools/scanpoc/out/gt-check"
os.makedirs(out_dir, exist_ok=True)


def draw_gt_overlay(img, gt_data, name):
    h, w = img.shape[:2]
    vis = img.copy()
    ...
    cv2.putText(vis, f"Ground Truth: {name} ({w}x{h})", (15, 38), cv2.FONT_HERSHEY_SIMPLEX, 1.1, (255, 255, 255), 2, cv2.LINE_AA)
    return vis
...
def main():
    for i in range(1, 13):
        ...
        with open(gt_path, "r", encoding="utf-8") as f:
            gt_data = json.load(f)
        overlay = draw_gt_overlay(img, gt_data, stem)
        out_path = os.path.join(out_dir, f"{stem}.gt-overlay.png")
